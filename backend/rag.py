...
class LocalRAG:
    """BM25 over pre-built `data/index/chunks.jsonl` with citation tags."""
    def __init__(self, index_path="backend/data/index/chunks.jsonl"):
        ...
        if not path.exists():
            self.recs = [
                ...
                {"id":"guide-002","domain":"sphere","source_title":"wash_safe_water","chunk":"Safe water point; handwashing; queue management; chlorine residual guidance."},
            ]
        else:
    ...
    def _tok(self, s): 
        import re; return re.findall(r"[a-z0-9]+", s.lower())
